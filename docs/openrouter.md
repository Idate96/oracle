...
- For built-in models (`gpt-5.1-pro`, `o3-deep-research`, `o4-mini-deep-research`, `claude-4.5-sonnet`, `gemini-3.1-pro`, `gemini-3-pro`), Oracle falls back to OpenRouter automatically when the provider key is missing but `OPENROUTER_API_KEY` is set. It keeps using first‑party endpoints when their keys are present.
