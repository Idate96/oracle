...
```json5
{
  ...
  browser: {
    ...
    composerMode: null, // set "deep-research" to enable ChatGPT Deep research by default
...
- Browser automation defaults can be set under `browser.*`, including `browser.manualLogin`, `browser.manualLoginProfileDir`, `browser.thinkingTime` (CLI override: `--browser-thinking-time`), and `browser.composerMode` (currently `"deep-research"`). On Windows, `browser.manualLogin` defaults to `true` when omitted.
...
- Defaults: `auto` = 60m for long-running API models (`gpt-5.5-pro`, Pro aliases, and Deep Research); non-pro API models use `120s` if you don’t set a value.
