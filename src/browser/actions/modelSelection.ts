      ...
      const attempt = async () => {
        ...
        if (match) {
          if (optionIsSelected(match.node)) {
            closeMenu();
            resolve({ status: 'already-selected', label: match.label || getButtonLabel() });
            return;
          }
          const clicked = dispatchClickSequence(match.node);
          if (!clicked) {
          ...
          // ChatGPT can keep the top-bar label generic ("ChatGPT") even after a valid model click.
          // If the menu closes or the option is marked selected, treat the click as a successful switch.
          setTimeout(() => {
            if (buttonMatchesTarget()) {
              closeMenu();
              resolve({ status: 'switched', label: match.label || getButtonLabel() });
              return;
            }
            const menuOpen = document.querySelector('[role="menu"], [data-radix-collection-root]');
            if (!menuOpen || optionIsSelected(match.node)) {
              closeMenu();
              resolve({ status: 'switched-best-effort', label: match.label || getButtonLabel() });
              return;
            }
            attempt();
