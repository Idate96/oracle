...
Oracle bundles your prompt and files so another AI can answer with real context. It speaks GPT-5.6 Pro through ChatGPT browser mode, GPT-5.5 Pro through the API (default), OpenAI Deep Research (`o3-deep-research` / `o4-mini-deep-research` via API, or ChatGPT Deep research via browser mode), GPT-5.4, GPT-5.1 Pro, GPT-5.1 Codex (API-only), GPT-5.1, GPT-5.2, Gemini 3.1 Pro (API-only), Gemini 3 Pro, Claude Sonnet 4.5, Claude Opus 4.1, and more—and it can ask one or multiple models in a single run. Browser automation is available; use `--browser-deep-research` to select ChatGPT's Deep research composer mode, `--browser-model-strategy current` to keep the active ChatGPT model, or `ignore` to skip the picker. API remains the most reliable path, and `--copy` is an easy manual fallback.

## Setting up (macOS Browser Mode)

Browser mode lets you use GPT-5.5 Pro without any API keys by selecting the ChatGPT `Pro` picker entry, setting `--browser-thinking-time extended`, and automating your Chrome browser directly.
...
# Deep Research API run (long-running; uses web search and background mode by default)
npx -y @steipete/oracle --engine api --model o3-deep-research \
  -p "Research current options for SOC 2 evidence automation and return a sourced comparison"

# ChatGPT browser Deep research run (selects the composer tools-menu mode)
npx -y @steipete/oracle --engine browser --browser-deep-research \
  -p "Research current options for SOC 2 evidence automation and return a sourced comparison"
...
- API mode expects API keys in your environment: `OPENAI_API_KEY` (GPT-5.x and API Deep Research), `GEMINI_API_KEY` (Gemini 3.1 Pro / Gemini 3 Pro), `ANTHROPIC_API_KEY` (Claude Sonnet 4.5 / Opus 4.1).
...
| `-m, --model <name>`                                            | Built-ins (`gpt-5.6-pro` for current ChatGPT browser Pro, `gpt-5.5-pro` API default, `o3-deep-research` / `o4-mini-deep-research` for API Deep Research, `deepresearch` as browser Deep research shorthand when browser engine is selected, `gpt-5.4-pro` legacy Pro alias, `gpt-5.4`, `gpt-5.1-pro`, `gpt-5-pro`, `gpt-5.1`, `gpt-5.1-codex`, `gpt-5.2`, `gpt-5.2-instant`, `gpt-5.2-pro`, `gemini-3.1-pro` API-only, `gemini-3-pro`, `claude-4.5-sonnet`, `claude-4.1-opus`) plus any OpenRouter id. |
...
| `--browser-model-label <label>`                                 | Exact ChatGPT model picker label to select in browser mode, e.g. `Pro`. Use `--browser-thinking-time extended` for GPT-5.6 Pro extended thinking.                                                                                                                                                                                                                                                                                                                                                      |
...
| `--browser-deep-research`                                       | Select ChatGPT `Deep research` from the composer tools menu before sending (browser mode). `--engine browser --model deepresearch` is a shorthand.                                                                                                                                                                                                                                                                                                                                                     |
